...
- ポストエフェクトは、フェーダー 0-7 を `u_faderValues[8]`、マスターを `u_faderMaster` として渡します。
- マスター（9本目）はデフォルトで Invert に割り当てています。
- MIDI 未接続時は、`cells[].keyboardKey` を設定したセルのみキーボード代替入力が有効です。
- `keyboardKey` を設定していないセルは、キーボード代替の対象になりません（MIDI入力は通常どおり利用可能）。

## デフォルトホットキー

- `Space`: フルスクリーン
- `Enter`: Tap Tempo
- `Shift`: Beat を 0 にリセット
...
詳細な MIDI 設定とキーボード代替入力の記述方法は `docs/midi.md` を参照してください。
