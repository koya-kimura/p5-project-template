import type { VisualRenderContext } from "../core/visualComposer";
...
export class DebugScreen {
    draw(ctx: VisualRenderContext): void {
        ...
        const midiFaders = midiManager.faderValues.map((value) => value.toFixed(2)).join(", ");
        const midiToggles = midiManager.faderButtonToggleState.map((state) => (state ? "1" : "0")).join(" ");
        ...
        const infoLines = [
            ...
            ...bandDefinitions.map((band) =>
                `${band.label}: ${band.triggered ? "ON" : "off"} (lvl ${band.level.toFixed(2)})`,
            ),
        ...
        // Volume meter and band meters (monochrome)
        tex.push();
        tex.fill(255);
        tex.textSize(14);
        tex.textAlign(p.LEFT, p.TOP);
